...
var data = `ckmb52fldxkseven3fkjgcbzmnr7
...
func main() {
	...
	for _, line := range splitted {
		coordinates := getNumbersFromString(line)

		coords = append(coords, coordinates)
	}

	fmt.Println(sum(coords))
}

func getNumbersFromString(input string) int {
	re := regexp.MustCompile("[0-9]")
	stringNumbers := re.FindAllString(input, -1)

	first, err := strconv.Atoi(stringNumbers[0])

	if err != nil {
		fmt.Println(err)
	}

	last, err := strconv.Atoi((stringNumbers[len(stringNumbers)-1]))
...
func joinDigits(a int, b int) int {
